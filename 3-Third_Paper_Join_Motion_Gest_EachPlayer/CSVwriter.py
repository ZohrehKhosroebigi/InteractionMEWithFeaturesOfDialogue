...
##################################
    ...
    def csv_wrt(self, rows, All_SCV_file):
        # csv header
        fieldnames = []
        for item in rows:
        ...
        # to avoid writing header several times


        file_exists = os.path.isfile (All_SCV_file)

        with open (All_SCV_file, 'a', encoding='UTF8', newline='') as allf:
            writer = csv.DictWriter (allf, fieldnames=fieldnames)
            if not file_exists:
                writer.writeheader ()
